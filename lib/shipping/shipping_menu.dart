...
class ShippingMenu extends StatefulWidget {
  @override
  _ShippingMenuState createState() => _ShippingMenuState();
}
...
class _ShippingMenuState extends State<ShippingMenu> {
  ...
  _getMe() async {
    String token = await store.read('token');
    if (token != null && await store.read('app') == 'shipping') {
      var response = await getMe('shipping/auth/me', token);
      if (response != null) {
  ...
  _navigate(context, path) {
    Navigator.of(context).pop();
    Navigator.pushNamed(context, path, arguments: 'shipping/');
  }
}
